...
module.exports = function commitCmd(message = '') {
  ...
  if (shouldFull) {
    ...
    const outFile = path.join(apiverRoot, 'snapshots', `${versionName}.full.apiver`); // Corrected path
    ...
    if (!baseVersion) {
      ...
      try {
        ...
        fs.writeFileSync(path.join(apiverRoot, 'patches', patchFileName), encPatch); // Corrected path
