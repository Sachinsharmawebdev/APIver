...
const { compressAndEncrypt, decompressAndDecrypt } = require('./utils/crypto');

module.exports =
  function hotfixFile(arg1, arg2, arg3) {
  // New style: “<name> from <version>”
  if (arg2 === 'from' && arg3) {
    const hotfixName  = arg1;
    const baseVersion = arg3;
    ...
    if (!Array.isArray(meta.hotfixes[baseVersion])) meta.hotfixes[baseVersion] = [];
    meta.hotfixes[baseVersion].push(`${hotfixName}.patch.apiver`);
    fs.writeFileSync(metaPath, JSON.stringify(meta, null, 2));

    console.log(`Created hotfix: ${hotfixName}`);
    return;
  }

  // Legacy style keeps original behaviour
  const version  = arg1;
  const filePath = arg2;
  const metaPath = path.join('.APIver', 'meta.json');
  if (!fs.existsSync(metaPath)) {
